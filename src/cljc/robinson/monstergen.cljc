...
(def ^:private monsters
  ...
  ;;        |               |  |  |                |                |    | | |     |     |    | |                                           |                             |                       |                                       | status
  (Monster. :rat            1  90 "rat"            "rats"           nil  5 0 0.9   0.2   2    2 #{:face :head :neck :body :leg :tail}       #{:bite :claw}                :hostile                :follow-player-in-range-or-random       8 #{:hostile})
  (Monster. :gecko          3 110 "gecko"          "geckos"         nil  7 0 0.9   0.2   3    2 #{:head :face :body :tail :leg}             #{:bite}                      :hostile-after-sound    :follow-player-in-range-or-random       4 #{:hostile})
  (Monster. :bird           5 130 "bird"           "birds"          nil  9 0 2.1   1     3    2 #{:head :body :tail :leg :beak :wing}       #{:bite :claw}                :retreat-after-sound    :random                                 8 #{:docile})
  (Monster. :mosquito       2 180 "mosquito"       "mosquitoes"     nil  5 0 1.4   0.01  3    1 #{:head :body :leg :wing}                   #{:bite}                      :hostile-after-attacked :follow-player-in-range-or-random       3 #{:hostile})
  (Monster. :spider         2 120 "spider"         "spiders"        nil  8 0 0.9   0.1   9    4 #{:face :leg :abdomen}                      #{:bite-venom}                :retreat-after-attacked :follow-player-in-range-or-random       5 #{:docile})
