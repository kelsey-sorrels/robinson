...
(def ^:private monsters
  ...
  ;;        |               |  |  |                |                |    | | |     |     |    | |                                           |                             |                       |                                       | status
  (Monster. :rat            1  90 "rat"            "rats"           nil  6 0 0.9   0.2   2    1 #{:face :head :neck :body :leg :tail}       #{:bite :claw}                :hostile                :follow-player-in-range-or-random       8 #{:hostile})
  (Monster. :gecko          1 110 "gecko"          "geckos"         nil  9 0 0.9   0.2   2    1 #{:head :face :body :tail :leg}             #{:bite}                      :hostile-after-sound    :follow-player-in-range-or-random       4 #{:hostile})
