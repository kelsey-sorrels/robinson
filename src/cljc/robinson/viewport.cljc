        ...
        conj-cells (fn [cells place min-x min-y max-x max-y x-range y-range]
                     (reduce (fn [cells [line sy]]
                             ...
                             (map vector
                                  ...
                                  y-range)))
        cells (-> (transient [])
                    (conj-cells ul-place start-x start-y v-width v-height r-x     r-y)
                    (conj-cells ur-place 0       start-y start-x v-height r-xrest r-y)
                    (conj-cells ll-place start-x 0       v-width start-y  r-x     r-yrest)
                    (conj-cells lr-place 0       0       start-x start-y  r-xrest r-yrest)
                    persistent!)]
    ;(log/info "ul-place-id" ul-place-id)
    ;(log/info "ur-place-id" ur-place-id)
    ;(log/info "lr-place-id" lr-place-id)
