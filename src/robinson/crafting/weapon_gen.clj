...
(defn after-item-event
  [item]
  {:description (i/<< "You hold the ~(get item :name) in your hand, examining the weight, size, and makeup. You start to think about how to use it.")
   :event/choices [
     {:choice/events [:next-event]}]})
        ...
        choices (map (fn [item]
                  (let [choice {:hotkey (get item :hotkey)
                    ...
                    (if-let [valid-recipes (not-empty (rcrafting/valid-recipes (conj (get recipe :$/items) item)
                      ...
                      (assoc choice :choice/events [(wrap-up-event recipe valid-recipes item)])
                      ; if item does not complete recipe, pick next event
                      (assoc choice :choice/events [(after-item-event item)]))))
...
(rs/def-storylet flexible-complication
  flexible-unused-item?
  (let [item (rand-nth (flexible-unused-items state))]
    {:event/id :event.id/flexible
     :title (get item :name)
     :description [(i/<< "The ~(get item :name) you are working with is too ridged You need a more supple part to move on.")
...
(defn next-event
  ...
  (let [after :none]
    {:event/id :next-event
     ...
     :event/choices
       [{:name "continue"
         :choice/events [(rs/rand-storylet state recipe
...
(rs/def-storylet psychological-event-optimism
  (rs/in-history? :choice.id/mental-optimism)
...
(rs/def-storylet psychological-event-self-control
  (rs/in-history? :choice.id/mental-self-control)
